...
gallery.innerHTML = galleryListItems;
...
const galleryHoverHandler = (event) => {
    if (event.target.tagName === "IMG") {
        event.target.style.transition = "transform 1000ms cubic-bezier(0.075, 0.82, 0.165, 1)";
        event.target.style.transform = "scale(1.04)";
    }
};
const galleryUnHoverHandler = (event) => {
    if (event.target.tagName === "IMG") {
...
gallery.addEventListener("mouseover", galleryHoverHandler);
gallery.addEventListener("mouseout", galleryUnHoverHandler);
